...
function module:start()
    ...
    Main.Position = UDim2.new(0.5, 150, 0.5, 0) -- Shifted right to make space for debug frame
    ...
    -- Debug Frame Setup (Left Side with Blur)
    DebugFrame.Name = "DebugFrame"
    DebugFrame.Parent = UI
    DebugFrame.BackgroundTransparency = 1
    DebugFrame.Position = UDim2.new(0.5, -495, 0.5, 0) -- Left of Main
    DebugFrame.Size = UDim2.new(0, 300, 0, 435)
    DebugFrame.AnchorPoint = Vector2.new(0.5, 0.5)

    DebugBackground.Name = "DebugBackground"
    DebugBackground.Parent = DebugFrame
    DebugBackground.BackgroundColor3 = Color3.fromRGB(17, 17, 17)
    DebugBackground.BackgroundTransparency = 0.45
    DebugBackground.Size = UDim2.new(1, 0, 1, 0)
    DebugBackground:SetAttribute("BlurIntensity", START_INTENSITY)
    CollectionService:AddTag(DebugBackground, "DropShadow")
    DebugCorner.Parent = DebugBackground

    DebugScroll.Name = "DebugScroll"
    DebugScroll.Parent = DebugBackground
    DebugScroll.BackgroundTransparency = 1
    DebugScroll.Position = UDim2.new(0, 5, 0, 5)
    DebugScroll.Size = UDim2.new(1, -10, 1, -10)
    DebugScroll.CanvasSize = UDim2.new(0, 0, 0, 0) -- Will auto-resize
    DebugScroll.ScrollBarThickness = 5

    DebugText.Name = "DebugText"
    DebugText.Parent = DebugScroll
    ...
    DebugText.FontFace = Font.new("rbxassetid://12187365977") -- Same font face
    DebugText.TextSize = 14
    DebugText.Text = "Debug Console Initialized\n"

    -- Override print to log to DebugText
    local originalPrint = print
    print = function(...)
        local args = {...}
        local message = table.concat(args, " ")
        originalPrint(message) -- Still prints to default console
        DebugText.Text = DebugText.Text .. message .. "\n"
        DebugScroll.CanvasSize = UDim2.new(0, 0, 0, DebugText.TextBounds.Y + 10)
        DebugScroll.CanvasPosition = Vector2.new(0, DebugText.TextBounds.Y) -- Auto-scroll to bottom
    end
    ...
    -- Setup blur effect for AcrylicBackground
    do
    ...
    -- Setup blur effect for DebugBackground
    do
        local blurPart = Instance.new("Part")
        blurPart.Size = Vector3.new(1, 1, 1) * PART_SIZE
        blurPart.Anchored = true
    ...
    RunService:BindToRenderStep("BlurUpdate", Enum.RenderPriority.Camera.Value + 1, function()
        for blurObj in pairs(BlurObjects) do
            blurObj.Part.CFrame = camera.CFrame
        end
        updateAll()
    end)
    ...
    Rejoin.Name = "Rejoin"
    Rejoin.Parent = UserContainer
    Rejoin.AnchorPoint = Vector2.new(1, 0.5)
    Rejoin.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    Rejoin.BackgroundTransparency = 0.900
    ...
    ServerHop.Parent = UserContainer
    ServerHop.AnchorPoint = Vector2.new(1, 0.5)
    ...
    UserLabel.Text = "Welcome <b>" .. Players.LocalPlayer.Name .. "</b>!"
    ...
    function start:AddPlayer(player, inventoryCost, isElite)
        ...
        if playerToAdd then
            ...
            TargetIcon.Image = Players:GetUserThumbnailAsync(playerToAdd.UserId, thumbType, thumbSize)
